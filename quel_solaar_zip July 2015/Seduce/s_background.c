...
void seduce_background_particle_init()
{
	...
	SBackgroundRender.light_texture = r_texture_allocate(R_IF_RGBA_FLOAT16, 256, 256, 1, TRUE, FALSE, NULL);
...
void seduce_background_particle_update(BInputState *input)
{
	...
	for(i = 0; i < S_BACKGROUND_PARTICLE_COUNT; i++)
	{
		if(SBackgroundRender.particles[i].age < 1.0)
		{
			...
			SBackgroundRender.particles[i].vector[0] *= (1 -  input->delta_time * 3);
			SBackgroundRender.particles[i].vector[1] *= (1 -  input->delta_time * 3);
		}
	}
	for(i = 0; i < input->pointer_count; i++)
	{
...
void seduce_background_particle(BInputState *input)
{
	...
	//	glBlendFunc(GL_ONE, GL_ONE);
		...
		f_hsv_to_rgb(rgb, input->minute_time, 0.4, 0.3);
		for(i = 0; i < input->pointer_count; i++)
...
void s_background_shader_set(BInputState *input, float time, float *center)
{
	...
	if(time	< 0.999)
	{
		...
		if(center == NULL)
			...
			r_shader_vec2_set(shader, r_shader_uniform_location(shader, "center"), center[0] * 0.5, center[1] * 0.5 / betray_screen_mode_get(NULL, NULL, NULL));
	...
	if(input->mode == BAM_DRAW && time > 0.0)
	{
		...
		}else
		{
			...
			if(center == NULL)
				...
				r_shader_vec2_set(s_background_shader_mask, r_shader_uniform_location(s_background_shader_mask, "center"), center[0] * 0.5, center[1] * 0.5 / aspect);
...
void seduce_background_angle_set(BInputState *input, void *id, float angle_a, float angle_b, boolean active)
{
	...
	seduce_background_quad_draw(input, id, x[0] * 0.08 + y[0] * 0.005, y[0] * 0.08 - x[0] * 0.005, 0, 
											x[0] * 0.2 + y[0] * 0.005, y[0] * 0.2 - x[0] * 0.005, 0, 
											x[1] * f, y[1] * f, 0, 
											x[1] * 0.08, y[1] * 0.08, 0, 
											y[0] * 0.1, -x[0] * 0.1, 1,
											color[0] - 0.2, color[1] - 0.2, color[2] - 0.2, color[3]);
	seduce_background_quad_draw(input, id, x[1] * 0.08, y[1] * 0.08, 0, 
											x[1] * f, y[1] * f, 0, 
...
void seduce_background_betweeners_set(BInputState *input, float angle)
{
	...
	seduce_background_quad_draw(input, NULL, x * 0.2 - y * 0.005, y * 0.2 + x * 0.005, 0, 
...
void seduce_background_circle_draw(BInputState *input, float pos_x, float pos_y, uint splits, float timer, uint selected)
{
	if(input->mode == BAM_DRAW)
	{
		...
		r_matrix_projection_screenf(r_matrix_get(), output, 0, 0, 0);
		seduce_background_polygon_flush(input, output, 0.1 + timer * 0.5);
	}
}



boolean seduce_background_angle(BInputState *input, void *id, float pos_x, float pos_y, float angle_a, float angle_b, float timer)
{
	uint i;
	if(input->mode == BAM_DRAW)
	{
...
/*		if(active)
		...
		seduce_element_add_quad(input, id, 0, &box[0], &box[3], &box[6], &box[9]);
		if(angle_b < PI / 3)
		{
			box[0] = sin(angle_a);
			box[1] = cos(angle_a);
			...
			if(active)
			{
				seduce_background_quad_draw(input, id,
										...
										0.2, 0.6, 1.0, 0.9);
				seduce_background_tri_draw(input, id,
										box[0] * 0.25 + box[1] * 0.005, box[1] * 0.25 - box[0] * 0.005, 0, 
...
boolean seduce_background_tri_draw(BInputState *input, void *id,
														...
														float surface_r, float surface_g, float surface_b, float surface_a)
{
	...
	if(input->mode == BAM_DRAW)
	{
		...
		if(id != NULL)
		{
			seduce_element_add_triangle(input, id, 0, array, &array[S_BACKGROUND_VERTEX_SIZE * 1], &array[S_BACKGROUND_VERTEX_SIZE * 2]);
...
boolean seduce_background_quad_draw(BInputState *input, void *id,
														float a_x, float a_y, float a_z, 
														float b_x, float b_y, float b_z, 
														float c_x, float c_y, float c_z, 
														...
														float surface_r, float surface_g, float surface_b, float surface_a)
{
	if(seduce_background_tri_draw(input, id, a_x, a_y, a_z, 
											b_x, b_y, b_z, 
											c_x, c_y, c_z, 
											normal_x, normal_y, particle_influence,
											surface_r, surface_g, surface_b, surface_a) ||
		seduce_background_tri_draw(input, id, a_x, a_y, a_z, 
											c_x, c_y, c_z, 
											d_x, d_y, d_z, 
											normal_x, normal_y, particle_influence,
											surface_r, surface_g, surface_b, surface_a))
