...
void seduce_view_rotate(SViewData *v, float x, float y)
{
	if(v == NULL)
		v = &sui_default_view;	
	seduce_view_orbit_matrix(v, x, y);
...
void seduce_view_change_right_button(SViewData *v, BInputState *input)
{
...
void seduce_view_change_multitouch(SViewData *v, BInputState *input)
{
...
void seduce_view_projection_line_snap(double *output, uint axis, double direction, double *start, double *snap, SUISnapType snap_type)
{
/*	output[0] = start[0];
	output[1] = start[1];
	output[2] = start[2];
