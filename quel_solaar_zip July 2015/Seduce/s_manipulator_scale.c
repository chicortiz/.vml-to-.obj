...
boolean	seduce_manipulator_radius(BInputState *input, RMatrix *m, float *pos, float *radius, void *id, float time)
{
	...
	if(input->mode == BAM_EVENT)
	{
		for(i = 0; i < input->pointer_count; i++)
		{
			...
			if(grab[i].id == id && grab[i].active)
			{	
				r_matrix_projection_vertexf(m, tmp, pos, input->pointers[i].pointer_x, input->pointers[i].pointer_y);
				tmp[0] = tmp[0] - pos[0];
				tmp[1] = tmp[1] - pos[1];
				tmp[2] = tmp[2] - pos[2];
	...
	if(input->mode == BAM_EVENT)
	{
		for(i = 0; i < input->pointer_count; i++)
		{
			...
			if(grab[i].id == id && grab[i].active)
			{
				...
				{
					if(r_matrix_projection_axisf(m, tmp, pos, grab[i].part, input->pointers[i].pointer_x, input->pointers[i].pointer_y))
					{
				...
				}else
				{
					...
					r_matrix_projection_worldf(m, vector, 0, 0, 0);
					for(j = 0; j < 3; j++)
					{
						vector[j] -= pos[j];
						if(vector[j] > 0)
					...
					r_matrix_projection_vectorf(m, tmp, pos, vector, input->pointers[i].pointer_x, input->pointers[i].pointer_y);
					tmp[0] = tmp[0] - pos[0];
					tmp[1] = tmp[1] - pos[1];
					tmp[2] = tmp[2] - pos[2];
			...
			}else if(!grab[i].active)
			{
				if(input->pointers[i].button[0] && !input->pointers[i].last_button[0])
				{
					if(id == seduce_element_pointer_id(input, i, &grab[i].part))
					{
						...
						if(grab[i].part < 3)
						{
							r_matrix_projection_axisf(m, tmp, pos, grab[i].part, input->pointers[i].pointer_x, input->pointers[i].pointer_y);
						...
						}else
						{
							...
							if(grab[i].part < 6)
								vector[grab[i].part % 3] = 0.0;
							if(r_matrix_projection_vectorf(m, tmp, pos, vector, input->pointers[i].pointer_x, input->pointers[i].pointer_y))
							{
								tmp[0] = tmp[0] - pos[0];
								tmp[1] = tmp[1] - pos[1];
								tmp[2] = tmp[2] - pos[2];
				...
				if(grab[i + pointer_count].id == id && grab[i + pointer_count].active)
				{
					...
					else 
					{
						...
						if(axis != -1)
						{
							...
							r_matrix_projection_screenf(m, tmp, pos[0] - vec[0], pos[1] - vec[1], pos[2] - vec[2]);
							r_matrix_projection_screenf(m, tmp2, pos[0] + vec[0], pos[1] + vec[1], pos[2] + vec[2]);
							tmp[0] -= tmp2[0];
							tmp[1] -= tmp2[1];
							f_normalize2f(tmp);
