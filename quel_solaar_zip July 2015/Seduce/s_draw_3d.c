...
//		seduce_object_3d_shader = r_shader_create_simple(buf, 5000, crystal_shader_vertex, crystal_shader_fragment, "3Dobjects");
		printf(buf);
	}
	r_shader_set(seduce_object_3d_shader);
//	r_shader_attrib_bind(r_3d_shader, 0, "vertex");
//	r_shader_attrib_bind(r_3d_shader, 1, "normal");


//	seduce_object_3d_texture_shade = r_texture_gen(image_buffer/*seduce_object_3d_texture_shade_data*/, GL_UNSIGNED_BYTE, 64, 64, FALSE);
	seduce_object_3d_texture_shade = r_texture_allocate(R_IF_RGB_UINT8, 128, 128, 1, TRUE, TRUE, image_buffer);
	

//	seduce_object_3d_texture_shade = r_texture_allocate(R_IF_RGB_UINT8, 128, 128, 1, TRUE, FALSE, NULL/*seduce_object_3d_texture_shade_data*/);
	{
	/*	float *reflection, f;
		uint i;
		reflection = malloc((sizeof *reflection) * 49152);
		for(i = 0; i < 128 * 128; i++)
		{
			f = (seduce_object_3d_texture_reflection[i * 3 + 0] + seduce_object_3d_texture_reflection[i * 3 + 1] + seduce_object_3d_texture_reflection[i * 3 + 2]) / 3.0;
			f *= f;

			reflection[i * 3 + 0] = seduce_object_3d_texture_reflection_data[i * 3 + 0];
			reflection[i * 3 + 1] = seduce_object_3d_texture_reflection_data[i * 3 + 2];
			reflection[i * 3 + 2] = seduce_object_3d_texture_reflection_data[i * 3 + 1];
		}
		for(i = 0; i < 49152; i++)
			reflection[i] = reflection[i] * reflection[i] * reflection[i];*/
	//	seduce_object_3d_texture_reflection = r_texture_gen(seduce_object_3d_texture_reflection_data, GL_UNSIGNED_BYTE, 128, 128, FALSE);
	}
	...
	seduce_object_3d_color(0.5, 0.5, 0.5, 
							0.05, 0.05, 0.05,
							1.0, 1.0, 1.0);
/*	r_object_color(0.05, 0.05, 0.05, 
		0.5, 0.5, 0.5,
		1.0, 1.0, 1.0);*/
//	r_object_color(0.9, 0.9, 0.9, 0.2, 0.1, 0.3, 0.2, 0.6, 1.0);
	sui_3d_export();
}
...
void sui_3d_export()
{
	...
	for(i = 0; i < SEDUCE_DRAW_3D_COUNT; i++)
	{
		fprintf(file, "<node-geometry id=\"n%u\" name=\"Geometry_Node_1\">\n", i, "name");
		fprintf(file, "\t<layers>\n");
