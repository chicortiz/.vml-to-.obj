...
//	shader->mode = 0;
	...
	shader->name[i] = 0;
	input_count = r_parse_shaders(output, shaders, stages, stage_count, input, shader->mode, 400, FALSE, instance_block, &shader->instance_block);
		...
		if(input[i].qualifyer == R_SIQ_BLOCK)
		{
			...
			if(shader->mode != R_SIM_FLAT)
			{
				if(shader->instance_block == shader->block_count)
					...
					j = bind_point++;
				r_glUniformBlockBinding(shader->program, r_glGetUniformBlockIndex(shader->program, shader->blocks[shader->block_count].name), j);
