...
void la_do_init(void)
{
	...
	GlobalOverlay.delete_array_length = NULL;
	...
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.line_shader, "base_color"), 0.8, 2.4, 3.2, 0.5);
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.line_shader, "fog_color"), 0.3, 0.0, 0.3, 0.5);
	...
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.poly_shader, "color_a"), 0.0, 0.0, 0.02, 0.5);
	r_shader_vec4_set(NULL, r_shader_uniform_location(GlobalOverlay.poly_shader, "color_b"), 0.04, 0.07, 0.1, 0.3);
	...
	if(GlobalOverlay.delete_shader == NULL)
...
//			r_gl(GL_QUADS, GlobalOverlay.tag_unselect, 24, 2, 0.8, 0.8, 0.8, 0.0);
	...
	//	r_text(0.03, SEDUCE_T_SIZE * -0.5, SEDUCE_T_SIZE, SEDUCE_T_SPACE, tag[i].group, color, color, color, 1.0);
		seduce_text_line_draw(0.03, SEDUCE_T_SIZE * -0.5, SEDUCE_T_SIZE, SEDUCE_T_SPACE, tag[i].group, color, color, color, 1.0, -1);

		f = seduce_text_line_length(SEDUCE_T_SIZE, SEDUCE_T_SPACE, tag[i].group, -1);
		seduce_text_line_draw(0.05 + f, SEDUCE_T_SIZE * -0.5, SEDUCE_T_SIZE, SEDUCE_T_SPACE, tag[i].tag, color, color, color, 1.0, -1);
...
void draw_owerlay_vertex(void)
{
	...
	//	r_gl(GL_POINTS, vertex_array, vertex_count, 3, 1, 1, 1, 0.0);
	}
/*	if(vertex_reference_array != NULL)
	{
		r_gl(GL_POINTS, vertex_reference_array, vertex_reference_count, 3, 0.4, 0.15, 0.2, 0.0);
	}*/
	if(base_lines != NULL)
	{
//		r_gl(GL_LINES, base_lines, vertex_count * 2, 3, 0.04, 0.16, 0.2, 0.0);
	}
}
