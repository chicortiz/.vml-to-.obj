...
"	gl_FragColor += vec4(dot(center, center) / (1.0 + dot(shade, shade))) * vec4(1, 0.8, 0.6, 0);"
"}";
...
{
	SUIFormats types[3] = {SUI_FLOAT, SUI_FLOAT, SUI_FLOAT};
...
void la_pfx_create_spark(double *pos)
{
	...
	p->vector[0] = f_randnf(counter++) * p->size;
	p->vector[1] = (f_randnf(counter++) + 0.5) * p->size;
	p->vector[2] = f_randnf(counter++) * p->size;
	p->age = 0;
}
...
//				r_set_vec3(GlobalParticleData.dust_pos, i * 2 + 1, p->pos[0], p->pos[1], p->pos[2]);
	...
	/*		glDepthMask(0);
			...
			r_gl(GL_LINES, GlobalParticleData.dust_pos, GlobalParticleData.dust_length * 2, 3, 0, 0, 0, 0.0);
			glDepthMask(1);*/
		}
	}
...
void lo_pfx_wiggle_circle(float x, float y, float z, float normal_x, float normal_y, float normal_z, float radius, float expand, boolean major)
{
	...
	for(i = 0; i < splits; )
	{
		...
		f_wiggle3df(&array[3], seed + move * 1.0, 0.5 * 4.0 * size * f * (1.0 - f) / speed);
		f_wiggle3df(&array[6], seed + move * 7.0, 0.1 * 4.0 * size * f * (1.0 - f) / speed);
		...
		if(f > 0.5)
			...
			move2 = f;  
		f_wiggle3df(tmp, seed + move2 * 1.0, 0.5 * size * f2 * (1.0 - f2) / speed);
		array[3] += tmp[0] + (array[0] - x) * expand;
		array[4] += tmp[1] + (array[1] - y) * expand;
		array[5] += tmp[2] + (array[2] - z) * expand;
		f_wiggle3df(tmp, seed + move2 * 7.0, 0.1 * size * f2 * (1.0 - f2) / speed);
		array[6] += tmp[0];
		array[7] += tmp[1];
		array[8] += tmp[2];
		i++;
		...
		f_wiggle3df(&array[12], seed + move * 1.0, 0.5 * 4.0 * size * f * (1.0 - f) / speed);
		f_wiggle3df(&array[15], seed + move * 7.0, 0.1 * 4.0 * size * f * (1.0 - f) / speed);
	...
	switch(snap_type)
	{
		...
		case SUI_ST_TANGENT :
			...
			if(action)
				...
				lo_pfx_select_circle(p[0], p[1], p[2], normal[0], normal[1], normal[2], f, 0, action);
			normal[0] += p[0];
			normal[1] += p[1];
			normal[2] += p[2];
			p2[0] = pos[0];
...
void lo_pfx_wiggle_draw(float delta)
{
	...
	for(i = 0; i < WIGGLE_BLOCK_COUNT; i++)
	{
		if(GlobalParticleData.wiggle_blocks[i].wiggle_major)
			GlobalParticleData.wiggle_blocks[i].wiggle_timer += delta * 0.125;
		else
			GlobalParticleData.wiggle_blocks[i].wiggle_timer += delta * 0.25;
		...
		if(GlobalParticleData.wiggle_blocks[i].wiggle_timer > 0.1)
		{
			GlobalParticleData.wiggle_blocks[i].wiggle_draw_lenght = 0;
			GlobalParticleData.wiggle_blocks[i].wiggle_used = 0; 
		}else
		{
			...
			if(GlobalParticleData.wiggle_blocks[i].wiggle_timer > 0.8)
			{
				r_shader_float_set(GlobalParticleData.wiggle_shader, r_shader_uniform_location(GlobalParticleData.wiggle_shader, "scale"), size * (1.0 - (GlobalParticleData.wiggle_blocks[i].wiggle_timer - 0.8) * 5.0));
				r_shader_float_set(GlobalParticleData.wiggle_shader, r_shader_uniform_location(GlobalParticleData.wiggle_shader, "cutoff"), size * (1.0 - (GlobalParticleData.wiggle_blocks[i].wiggle_timer - 0.8) * 5.0));
			}else
			{
			...
			if(GlobalParticleData.wiggle_blocks[i].wiggle_major)
				...
				r_shader_vec4_set(GlobalParticleData.wiggle_shader, r_shader_uniform_location(GlobalParticleData.wiggle_shader, "color"), 0.5 * 0.1, 0.6 * 0.1, 0.7 * 0.1, 0.3);
