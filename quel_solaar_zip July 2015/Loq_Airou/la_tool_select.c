	...
	for(i = 0; i < poly; i++)
	{
		...
		if(next != -1 && crease[polygon * 4 + i] < 4294967295 / 2)
		{
			...
			if(list[next] == 0)
			{
				...
				{
					case LA_PSST_PLANE :
					f_surface_cross3d(vec, &vertex[ref[next * 4] * 3], &vertex[ref[next * 4 + 1] * 3], &vertex[ref[next * 4 + 2] * 3]);
					f_normalize3d(vec);
...
void la_t_poly_surface_select(uint polygon, LAPolySurfaceSeletcType type)
{
	...
	if(type != LA_PSST_SMOOTH)
		crease = ref; 
	f_surface_cross3d(normal, &vertex[ref[polygon * 4] * 3], &vertex[ref[polygon * 4 + 1] * 3], &vertex[ref[polygon * 4 + 2] * 3]);
		...
		while(found)
		{
			...
			for(i = 0; i < ref_count; i++)
			{
				if(list[i] == 1)
				{
					...
					{
						f_surface_cross3d(normal, &vertex[ref[i * 4] * 3], &vertex[ref[i * 4 + 1] * 3], &vertex[ref[i * 4 + 2] * 3]);
						f_normalize3d(normal);
						if(la_t_poly_surface(list, i, ref, n, crease, vertex, vertex_count, normal, type))
							found = TRUE;	
					}
