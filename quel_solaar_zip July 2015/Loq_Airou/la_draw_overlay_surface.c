...
"	gl_FragColor = texture2D(image, uv.xy) + vec4(vec3(max(f / dist, 0.0)), 1.0) * vec4(1, 0.7, 0.3, 1.0) + vec4(0.4 / length(v) / scale) * manipulator_color;"
...
"	uv = normalize(n);"
"	freznel = dot(uv.xy, uv.xy);"
"	gl_FragColor = mix(vec4(0.3, 0.4, 0.8, 1), vec4(0.7, 0.5, 0.6, 1), n.y * -0.5 + 0.5) + vec4(1.0 / length(v) / scale) * manipulator_color;"		
...
LoOverlaySufaceGen	lo_preview_surface;
boolean				lo_preview_working;
...
boolean draw_persuade_surface_ready_preview()
{
	static float light_pos[3] = {0, 0, 0}, light_distance = 1, brightness = 1;
	float goal[3];
	uint v, i;

	if(lo_preview_working)
	{
		if(imagine_mutex_lock_try(lo_preview_surface.mutex))
		{

			SUIFormats types[2] = {SUI_FLOAT, SUI_FLOAT};
			uint  vertex_size[2] = {3, 3}, size, ref_size, vertex_count;
			size = r_array_vertex_size(types, vertex_size, 2);
			if(lo_preview_surface.job == LO_OSJT_SIMPLE)
			{
				if(lo_preview_surface.output_vertex_array != NULL)
				{
					lo_preview_pool = r_array_allocate(lo_preview_surface.vertex_count, types, vertex_size, 2, 0);
					r_array_load_vertex(lo_preview_pool, NULL, lo_preview_surface.output_vertex_array, 0, lo_preview_surface.vertex_count);	
					if(lo_preview_surface.output_vertex_array != NULL)
						free(lo_preview_surface.output_vertex_array);
					lo_preview_surface.output_vertex_array = NULL;
					lo_preview_vertex_section = NULL;
					lo_preview_ref_section = NULL;
					lo_preview_ref_length = lo_preview_surface.vertex_count;
				}
			}else if(lo_preview_surface.mesh != NULL)
			{
				lo_preview_pool = r_array_allocate(persuade2_lod_vertex_length_get(lo_preview_surface.mesh), types, vertex_size, 2, p_rm_get_ref_length(lo_preview_surface.mesh));
				lo_preview_vertex_section = r_array_section_allocate_vertex(lo_preview_pool, persuade2_lod_vertex_length_get(lo_preview_surface.mesh));
				lo_preview_ref_section = r_array_section_allocate_reference(lo_preview_pool, p_rm_get_ref_length(lo_preview_surface.mesh));
				lo_preview_ref_length = p_rm_get_ref_length(lo_preview_surface.mesh);
				if(lo_preview_ref_length == 0)
					lo_preview_ref_length = 0;
				r_array_load_vertex(lo_preview_pool, lo_preview_vertex_section, lo_preview_surface.output_vertex_array, 0, persuade2_lod_vertex_length_get(lo_preview_surface.mesh));
				r_array_load_reference(lo_preview_pool, lo_preview_ref_section, lo_preview_vertex_section, p_rm_get_reference(lo_preview_surface.mesh), p_rm_get_ref_length(lo_preview_surface.mesh));
				persuade_lod_destroy(lo_preview_surface.mesh);
				lo_preview_surface.mesh = NULL;
				persuade_mesh_destroy(lo_preview_surface.store);
				lo_preview_surface.store = NULL;
			}else
				lo_preview_ref_length = 0;

			imagine_mutex_unlock(lo_preview_surface.mutex);
			lo_preview_working = FALSE;
			return TRUE;
		}
	}
	return FALSE;
...
boolean draw_persuade_surface_draw_preview()
{
	...
	size = lo_preview_surface.max[0] - lo_preview_surface.min[0];
	if(size < lo_preview_surface.max[1] - lo_preview_surface.min[1])
		size = lo_preview_surface.max[1] - lo_preview_surface.min[1];
	if(size < lo_preview_surface.max[2] - lo_preview_surface.min[2])
		size = lo_preview_surface.max[2] - lo_preview_surface.min[2];
	scale = 1.25 / size;
	r_matrix_rotate(NULL, 30, 1, 0, 0);
	r_matrix_rotate(NULL, 30, 0, 1, 0);
	r_matrix_scale(NULL, scale, scale, scale);
	r_matrix_translate(NULL, (lo_preview_surface.min[0] + lo_preview_surface.max[0]) * -0.5,
							(lo_preview_surface.min[1] + lo_preview_surface.max[1]) * -0.5,
							(lo_preview_surface.min[2] + lo_preview_surface.max[2]) * -0.5);
	...
	for(i = 0; i < 2; i++)
	{
		...
		r_shader_float_set(shader[i], r_shader_uniform_location(shader[i], "scale"), scale);
...
{
	if(!lo_preview_working && node != NULL)
	{
		EGeoLayer *layer;
		uint32 *ref, *crease, i;
		double *vertex;
		lo_preview_surface.lod = 100.0;
		lo_preview_surface.min_tess = 1;
		lo_preview_surface.max_tess = 1;
		lo_preview_surface.center = TRUE;
		lo_preview_surface.ref_count = i = e_nsg_get_polygon_length(node);
		if(lo_preview_surface.ref_count > 4000)
			lo_preview_surface.job = LO_OSJT_SIMPLE;
		else
			lo_preview_surface.job = LO_OSJT_STRUCTURE;
		lo_preview_surface.vertex_count = e_nsg_get_vertex_length(node);
		...
		for(i = lo_preview_surface.ref_count; i < 1000; i += 4)
			lo_preview_surface.max_tess++;
		...
		if(layer != NULL)
			...
			crease = NULL;
		if(lo_preview_surface.ref != NULL)
			free(lo_preview_surface.ref);
		lo_preview_surface.ref = malloc((sizeof *lo_preview_surface.ref) * lo_preview_surface.ref_count * 4);
		memcpy(lo_preview_surface.ref, ref, (sizeof *lo_preview_surface.ref) * lo_preview_surface.ref_count * 4);

		if(lo_preview_surface.crease != NULL)
			free(lo_preview_surface.crease);
		lo_preview_surface.crease = malloc((sizeof *lo_preview_surface.crease) * lo_preview_surface.ref_count * 4);
		if(crease != NULL)
			memcpy(lo_preview_surface.crease, crease, (sizeof *lo_preview_surface.crease) * lo_preview_surface.ref_count * 4);
		else
			for(i = 0; i < lo_preview_surface.ref_count * 4; i++)
				lo_preview_surface.crease[i] = 0;

		if(lo_preview_surface.vertex_array != NULL)
			free(lo_preview_surface.vertex_array);
		lo_preview_surface.vertex_array = malloc((sizeof *lo_preview_surface.vertex_array) * lo_preview_surface.vertex_count * 3);
		for(i = 0; i < lo_preview_surface.vertex_count * 3; i++)
		{
			if(vertex[i] == V_REAL64_MAX)
				lo_preview_surface.vertex_array[i] = PERSUADE_ILLEGAL_VERTEX;
			else
				lo_preview_surface.vertex_array[i] = (float)vertex[i];
		}
		imagine_thread(lo_overlay_surface_func, &lo_preview_surface);	
		lo_preview_working = TRUE;
		return TRUE;
	}
	return FALSE;
}
void draw_persuade_surface_init()
{
	...
	lo_preview_surface.vertex_array = NULL;
	lo_preview_surface.vertex_count = 0;
	lo_preview_surface.ref = NULL;
	lo_preview_surface.ref_count = 0;
	lo_preview_surface.crease = NULL;
	lo_preview_surface.mutex = imagine_mutex_create();
	lo_preview_surface.store = NULL;
	lo_preview_surface.mesh = NULL;
	lo_preview_surface.output_vertex_array = NULL;
}
...
void draw_persuade_surface(BInputState *input)
{
	...
	if(!working)
	{
		...
		lo_overlay_surface.max_tess = imagine_setting_integer_get("MAX_TESS_LEVEL", 4, NULL);
		if(v != structure_version)
		{	
			uint32 *ref, *crease, limit, poly_count;
			double *vertex;
			lo_overlay_surface.job = LO_OSJT_STRUCTURE;
			lo_preview_surface.center = FALSE;
