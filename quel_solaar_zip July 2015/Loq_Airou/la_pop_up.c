...
boolean la_pu_object_selection(BInputState *input, boolean active)
{
	...
	for(node = e_ns_get_node_next(0, 0, V_NT_GEOMETRY); node != NULL; node = e_ns_get_node_next(e_ns_get_node_id(node) + 1, 0, V_NT_GEOMETRY))
	{
		...
		if(texture_id != -1 && e_search_node(node, search))
		{
			if(input->mode == BAM_DRAW)
			{
				...
				pos[0] = pos[0] * (1 - input->delta_time) + input->delta_time * ((float)(i % row_count) * 2.0 / (float)row_count - 1.0);
				pos[1] = pos[1] * (1 - input->delta_time) + input->delta_time * ((float)(i / row_count) * -2.0 / (float)row_count + aspect - (2 / (float)row_count));
				pos[2] = pos[2] * (1 - input->delta_time) + input->delta_time * (2.0 / (float)row_count);
	...
	if(input->mode == BAM_DRAW)
	{
		seduce_background_quad_draw(input, &grab_id,
	...
	if(seduce_widget_icon_button(input, &time, SUI_3D_OBJECT_CLOSE, 1 - 0.025, -aspect + 0.025,  0.025, time, color))
	{		
	...
	seduce_text_line_edit(input, &search, search, 64, 0.025 - 1.0, -aspect + 0.0125, 1, 0.025 * 0.5, seduce_translate("Search"), TRUE, NULL, NULL, 0.2, 0.6, 1.0, 1, 1, 0, 0, 1);
	return FALSE;
}
...
LAPUEmptyReturn la_pu_empty(BInputState *input, boolean active)
{
	...
	ring = seduce_popup(&fake_input, element, count, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
	switch(ring)
	{
		...
		break;
		case 2 :
			betray_requester_load(NULL, 0, element); 
		break;
		case 3 :
			betray_requester_save(NULL, 0, &element[1]);
		break;
		case 4 :
...
/*	if(input->mode == BAM_DRAW)
	{
		...
		if(tmp[0] * tmp[0] + tmp[1] * tmp[1] < 0.8 * 0.8)
			brightness = 1.0;
		seduce_text_line_draw(seduce_text_line_length(SEDUCE_T_SIZE / 2.0 * (16.0 * 1.6), SEDUCE_T_SPACE, "NEW", -1) * -0.5, 0, SEDUCE_T_SIZE / 2.0 * (16.0 * 1.6), SEDUCE_T_SPACE, "NEW", brightness, brightness, brightness, brightness, -1);
		glPopMatrix();
		glEnable(GL_BLEND);
		...
		for(node = e_ns_get_node_next(0, 0, V_NT_GEOMETRY); node != NULL; node = e_ns_get_node_next(e_ns_get_node_id(node) + 1, 0, V_NT_GEOMETRY))
		{
			...
			if((float)(i / 16) * 1.6 * (1.6 / (16.0 * 1.6)) - y > -aspect - 0.2)
			{
				...
				if(preview)
				{
					...
					{
						brightness = 0.4;
						seduce_text_line_draw(seduce_text_line_length(SEDUCE_T_SIZE / 2.0 * (16.0 * 1.6), SEDUCE_T_SPACE, e_ns_get_node_name(node), -1) * -0.5, - 0.8, SEDUCE_T_SIZE / 2.0 * (16.0 * 1.6), SEDUCE_T_SPACE, e_ns_get_node_name(node), 1.0, 1.0, 1.0, 1.0, -1);
					}
					glRotatef(rotate, 0, 1, 0);
					if(!lo_preview_geometry_draw(e_ns_get_custom_data(node, 0), brightness))
						seduce_text_line_draw(seduce_text_line_length(SEDUCE_T_SIZE / 2.0 * (16.0 * 1.6), SEDUCE_T_SPACE, "EMPTY", -1) * -0.5, 0, SEDUCE_T_SIZE / 2.0 * (16.0 * 1.6), SEDUCE_T_SPACE, "EMPTY", brightness, brightness, brightness, brightness, -1);
				}else
				{
					if(tmp[0] * tmp[0] + tmp[1] * tmp[1] < 0.8 * 0.8)
						brightness = 0.4;
					seduce_text_line_draw(seduce_text_line_length(SEDUCE_T_SIZE / 3.0 * (16.0 * 1.6), SEDUCE_T_SPACE, e_ns_get_node_name(node), -1) * -0.5, 0, SEDUCE_T_SIZE / 3.0 * (16.0 * 1.6), SEDUCE_T_SPACE, e_ns_get_node_name(node), brightness, brightness, brightness, brightness, -1);
				}
...
boolean la_pu_manipulator(BInputState *input, boolean active)
{
	...
	element[0].text = seduce_translate("Reverse");
	...
	ring = seduce_popup(&fake_input, element, 29, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
...
boolean la_pu_edge(BInputState *input, uint *edge, boolean active)
{
	...
	element[0].data.angle[1] = 360 / 9.0 * 1.0;
	element[1].type = S_PUT_ANGLE;
	element[1].text = seduce_translate("Revolve");
	element[1].data.angle[0] = 360 / 9.0 * 1.0;
	element[1].data.angle[1] = 360 / 9.0 * 2.0;
	element[2].type = S_PUT_ANGLE;
	element[2].text = seduce_translate("Tube");
	element[2].data.angle[0] = 360 / 9.0 * 2.0;
	element[2].data.angle[1] = 360 / 9.0 * 3.0;
	element[3].type = S_PUT_ANGLE;
	element[3].text = seduce_translate("Select Hull");
	element[3].data.angle[0] = 360 / 9.0 * 3.0;
	element[3].data.angle[1] = 360 / 9.0 * 4.0;
	element[4].type = S_PUT_ANGLE;
	element[4].text = seduce_translate("Collapse");
	element[4].data.angle[0] = 360 / 9.0 * 4.0;
	element[4].data.angle[1] = 360 / 9.0 * 5.0;
	element[5].type = S_PUT_ANGLE;
	element[5].text = seduce_translate("Measure Grid");
	element[5].data.angle[0] = 360 / 9.0 * 5.0;
	element[5].data.angle[1] = 360 / 9.0 * 6.0;
	element[6].type = S_PUT_ANGLE;
	element[6].text = seduce_translate("Select Fill");
	element[6].data.angle[0] = 360 / 9.0 * 6.0;
	element[6].data.angle[1] = 360 / 9.0 * 7.0;
	element[7].type = S_PUT_ANGLE;	
	element[7].text = seduce_translate("Define Axis Space");
	element[7].data.angle[0] = 360.0 / 9.0 * 7.0;
	element[7].data.angle[1] = 360.0 / 9.0 * 8.0;
	element[8].type = S_PUT_ANGLE;	
	element[8].text = seduce_translate("Strip fill Hole");
	element[8].data.angle[0] = 360.0 / 9.0 * 8.0;
	element[8].data.angle[1] = 360.0 / 9.0 * 9.0;
	...
	ring = seduce_popup(&fake_input, element, 9, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
	if(active)
...
boolean la_pu_polygon(BInputState *input, uint polygon, boolean active)
{
	...
	ring = seduce_popup(&fake_input, element, 11, time);
	free(pointers);
	if(input->mode == BAM_DRAW)	
		r_matrix_pop(NULL);
	if(active)
...
uint la_pu_select(BInputState *input, boolean active)
{
	...
	ring = seduce_popup(&fake_input, element, 4, time);
	free(pointers);
	if(input->mode == BAM_DRAW)
		r_matrix_pop(NULL);
	return ring;
