...
uint lo_preview_render(uint texture_id)
{
	...
	fbo = r_framebuffer_allocate(&texture_id, 1, depth_id);
	...
	{
		size = seduce_text_line_length(1, SEDUCE_T_SPACE, empty, -1);
		seduce_text_line_draw(-0.75, 0, 1.5 / size, SEDUCE_T_SPACE, empty, 1, 1, 1, 1, -1);
	}
...
void lo_preview_update()
{
	...
	if(draw_persuade_surface_ready_preview())
	{
		node = e_ns_get_node(0, current);
		if(node != NULL)
		{
	...
	for(node = e_ns_get_node_next(0, 0, V_NT_GEOMETRY); node != NULL; node = e_ns_get_node_next(e_ns_get_node_id(node) + 1, 0, V_NT_GEOMETRY))
	{
		...
		if(texture->version != e_ns_get_node_version_data(node) && current == -1)
		{
			if(draw_persuade_surface_queue_preview(node))
			{
				current = e_ns_get_node_id(node);
